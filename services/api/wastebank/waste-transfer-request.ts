...
import {
  CreateWasteTransferRequestParams,
  CreateWasteTransferResponse,
...
export const wasteBankTransferRequestAPI = {
  /**
   * Create waste transfer request
   * POST /api/waste-bank/waste-transfer-requests
   */
  async createWasteTransferRequest(
    params: CreateWasteTransferRequestParams
  ): Promise<CreateWasteTransferResponse> {
    try {
      const response = await authenticatedApiClient.post(
        '/api/waste-bank/waste-transfer-requests',
        params
      );
      ...
      if (axios.isAxiosError(error)) {
        const errorMessage =
          ...
          'Failed to create waste transfer request';
  ...
  /**
   * Assign collector to waste transfer request
   * PUT /api/waste-bank/waste-transfer-requests/:id/assign-collector
   */
  async assignCollectorToWasteTransferRequest(
    ...
    try {
      const response = await authenticatedApiClient.put(
        `/api/waste-bank/waste-transfer-requests/${id}/assign-collector`,
        params
      );
      return response.data;
    } catch (error) {
  ...
  /**
   * Update waste transfer request status
   * PUT /api/waste-bank/waste-transfer-requests/:id?status=:status
   */
  async updateWasteTransferRequestStatus(
    ...
    try {
      const response = await authenticatedApiClient.put(
        `/api/waste-bank/waste-transfer-requests/${id}?status=${params.status}`
      );
      return response.data;
    } catch (error) {
      if (axios.isAxiosError(error)) {
  ...
  /**
   * Complete waste transfer request
  ...
  async completeWasteTransferRequest(
    ...
    try {
      const response = await authenticatedApiClient.put(
        `/api/waste-bank/waste-transfer-requests/${id}/complete`,
        params
      );
      return response.data;
    } catch (error) {
